...
import { Search, Mail, Filter, MapPin, ArrowLeft, Loader2, User, Save, Phone, Upload, X } from 'lucide-react';
...
export const ClientDashboard = ({ onLogout }) => {
  ...
  const [profileData, setProfileData] = useState({ name: auth.currentUser?.displayName || "", phone: "", address: "" });
  ...
  const handleSaveProfile = async () => {
      ...
      setLoading(true);
      try {
          await updateDoc(doc(db, "users", auth.currentUser.uid), { name: profileData.name, phone: profileData.phone, address: profileData.address });
          setIsEditing(false);
  ...
  const filteredFirms = firms.filter(f => !searchTerm || f.name.toLowerCase().includes(searchTerm.toLowerCase()) || f.description?.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <DashboardLayout title="Marktplatz" user={auth.currentUser} sidebarItems={menu} activeTab={view} onTabChange={(t) => { setView(t); setSelectedFirm(null); }} onLogout={onLogout}>
          ...
          {loading ? <div className="flex justify-center p-10"><Loader2 className="animate-spin text-blue-600"/></div> : (
            <div className="space-y-4">
                ...
                <Card key={f.id} onClick={() => { setSelectedFirm(f); setView('details'); }} className="p-4 flex gap-4 cursor-pointer hover:border-blue-400 transition-all">
                    ...
                    <div><h3 className="font-bold text-lg text-slate-900">{f.name}</h3><div className="text-sm text-slate-500 flex gap-2 mb-2 items-center"><MapPin size={14}/> {f.address || "Keine Adresse"}</div><div className="text-blue-600 font-bold bg-blue-50 inline-block px-2 py-0.5 rounded text-sm">{f.showPrice !== false ? `ab ${f.price}€ / Std` : 'Preis auf Anfrage'}</div></div>
                </Card>
                ))}
            </div>
          )}
      ...
      {view === 'profile' && (
          <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in">
              <Card className="p-6 flex flex-col md:flex-row items-center gap-6 text-slate-900">
                  <div className="relative group">
                      ...
                      {isEditing && <label className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-xl cursor-pointer text-white animate-in fade-in"><Upload size={24}/><input type="file" className="hidden" onChange={handleImageUpload}/></label>}
                  </div>
                  <div className="text-center md:text-left flex-1"><h2 className="text-2xl font-bold">{profileData.name || "Kunde"}</h2><p className="text-slate-500">{auth.currentUser?.email}</p></div>
                  <Button variant={isEditing ? 'ghost' : 'primary'} onClick={() => setIsEditing(!isEditing)}>{isEditing ? 'Abbrechen' : 'Profil bearbeiten'}</Button>
              </Card>
              <Card className="p-6 space-y-6 text-slate-900">
                  <h3 className="font-bold text-lg border-b pb-2 mb-4">Meine Daten</h3>
                  <div className="grid grid-cols-1 gap-4">
                      <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Name</label><input disabled={!isEditing} className="w-full p-2 border rounded bg-white text-slate-900 disabled:bg-slate-50" value={profileData.name} onChange={e => setProfileData({...profileData, name: e.target.value})} /></div>
                      <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 flex items-center gap-1"><Phone size={12}/> Telefon</label><input disabled={!isEditing} className="w-full p-2 border rounded bg-white text-slate-900 disabled:bg-slate-50" value={profileData.phone} onChange={e => setProfileData({...profileData, phone: e.target.value})} /></div>
                      <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 flex items-center gap-1"><MapPin size={12}/> Adresse</label><input disabled={!isEditing} className="w-full p-2 border rounded bg-white text-slate-900 disabled:bg-slate-50" value={profileData.address} onChange={e => setProfileData({...profileData, address: e.target.value})} /></div>
                  </div>
                  {isEditing && <div className="pt-4 flex justify-end"><Button onClick={handleSaveProfile} disabled={loading} icon={Save}>Änderungen speichern</Button></div>}
              </Card>
          </div>
      )}
